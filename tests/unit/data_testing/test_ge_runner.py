...
def test_run_ge_tests_empty(alignment_config: AlignmentConfig, ge_test_folder_path: str):
    GERunner(alignment_config=alignment_config,
             ge_base_directory=ge_test_folder_path).run_ge_tests(named_tables=[], data_origin="FOO")
...
def test_run_ge_tests(
    ...
    GERunner(alignment_config=alignment_config, ge_base_directory=ge_test_folder_path).run_ge_tests(
