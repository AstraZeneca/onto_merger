...
class GERunner:
    ...
    def run_ge_tests(self, named_tables: List[NamedTable]) -> None:
        """Runs data tests for a list of named tables.
    ...
    def _configure_ge_context_data_sources(self, loaded_tables: List[NamedTable]) -> None:
        """Updates the data test context with the tables that are being tested.
    ...
    def _configure_ge_expectation_suites_for_entity(self, loaded_tables: List[NamedTable]) -> None:
        """Configures the data test expectation suites with tables that are being
        tested.
    ...
    def _produce_expectation_suite_for_entity(self, entity_name: str) -> ExpectationSuite:
        """Produces an expectation suite configuration (set of data tests) for a given
        table.
    ...
    def _run_validations(self, loaded_tables: List[NamedTable]) -> None:
        """Runs the data tests for a list of tables.
...
def block_print() -> None:
    """Blocks outputs to the console (GE produces many debug level outputs that
    otherwise cannot be stopped)."""
    sys.stdout = open(os.devnull, "w")


def enable_print() -> None:
    """Enables outputs to the console."""
